...
class Game{
    ...
    Game(playerHand in, int NS, uint numOpps = 1): 
    NumSims(NS),
    ...
    friend double printResults(std::vector< Game > v);
};
...
void Game::Serialsimulate(){
    int NumSims = this->NumSims;
...
void Threadedsimulate(std::vector< Game >& vec){
    ThreadPool tp;
    ...
    for(auto &inst: vec){
        tp.QueueJob(std::bind(&Game::Serialsimulate, &inst));
    }
...
double printResults (std::vector< Game > vec){

    std::cout << "\nUser Hand:\n" << vec[0].USER_HAND.first 
    << vec[0].USER_HAND.second << std::endl;

    double numWins = 0, totalSimulations = 0;
    for(auto &a: vec){
        numWins += a.numWins;
        totalSimulations += a.totalSimulations;
    }

    std::cout << "Probability of winning: "
    << numWins/totalSimulations << std::endl;

    return numWins/totalSimulations;
}